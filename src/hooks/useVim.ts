...
function calculateVisualRange(
  ...
  return {
    start: { row: minRow, col: visualStart.row <= cursor.row ? visualStart.col : minCol },
    end: { row: maxRow, col: visualStart.row <= cursor.row ? cursor.col : maxCol },
    type: 'char',
  };
}
...
function calculateOperatorRange(
  ...
  const forwardMotion =
    ...
    (motionResult.position.row === cursor.row && motionResult.position.col > cursor.col);
  ...
  if (forwardMotion) {
    return {
      start: cursor,
        ...
        : { row: motionResult.position.row, col: motionResult.position.col - 1 },
      type: 'char',
    };
  } else {
    return {
  ...
  const handleInput = useCallback(
    (char: string, key: Key): boolean => {
      ...
      if (char === 'd' || char === 'y' || char === 'c') {
        ...
        if (mode === 'operator-pending' && state.context.operator === operator) {
          // Linewise operation on current row
          const count = state.context.count || 1;
          const lineRange: Range = {
            start: { row: cursor.row, col: 0 },
            end: { row: Math.min(cursor.row + count - 1, config.dimensions.rows - 1), col: Infinity },
            type: 'line',
          };
      ...
      if (motionName) {
        const count = state.context.count || 1;
        const motionResult = executeMotion(motionName, count, cursor, config.motions);
        ...
        if (mode === 'operator-pending' && state.context.operator) {
          ...
          send({ type: 'MOTION', motion: motionName, position: motionResult.position });
          return true;
        }
      ...
      if (char === '.') {
        ...
        if (action && action.type === 'operator' && action.operator && action.motion) {
          const count = state.context.count || action.count || 1;
          const motionResult = executeMotion(action.motion, count, cursor, config.motions);
    ...
    [config, mode, send, state.context.count, state.context.operator, visualRange]
  );
